...
public class RunLengthEncoding implements Iterable {
    ...
    private DList redPix;
    private DList bluePix;
    private DList greenPix;
    private int width;
    private int height;
  ...
  public RunLengthEncoding(int width, int height) {
      redPix = new DList(0, width * height);
      bluePix = new DList(0, width * height);
      greenPix = new DList(0, width * height);
      this.width = width;
      this.height = height;
  }
  ...
  public RunLengthEncoding(int width, int height, int[] red, int[] green,
      ...
      for (int i = 0; i < runLengths.length; i++) {
          redPix.insertEnd(red[i], runLengths[i]);
          bluePix.insertEnd(blue[i], runLengths[i]);
          greenPix.insertEnd(green[i], runLengths[i]);
      }
  }
  ...
  public RunIterator iterator() {
    RunIterator iterate = new RunIterator(redPix.head, bluePix.head, greenPix.head);
    return iterate;
  ...
  public PixImage toPixImage() {
      ...
      int flag = 0;
      int[] copy = iterate.next();
      for (int i = 0; i < width; i++) {
          for (int j = 0; j < height; j++) {
              if (flag < copy[0]) {
                  result.setPixel(i, j, (short) copy[1], (short) copy[2], (short) copy[3]);
                  flag++;
              }
              else if (copy[0] == 0){
                  break;
  ...
  public RunLengthEncoding(PixImage image) {
      ...
      for (int i = 0; i < image.getWidth(); i++) {
          for (int j = 0; j < image.getHeight(); j++) {
              if(red[0] == image.getRed(i, j)) {
                  red[1]++;
              } else {
                  redPix.insertEnd(red[0], red[1]);
                  red[0] = image.getRed(i, j);
                  red[1] = 0;
              }
              if(blue[0] == image.getBlue(i, j)) {
                  red[1]++;
              } else {
                  bluePix.insertEnd(blue[0], blue[1]);
                  blue[0] = image.getBlue(i, j);
                  blue[1] = 0;
              }
              if(green[0] == image.getGreen(i, j)) {
                  green[1]++;
              } else {
                  greenPix.insertEnd(green[0], green[1]);
                  green[0] = image.getGreen(i, j);
                  green[1] = 0;
              }
          }
      }
